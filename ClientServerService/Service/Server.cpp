		...
		if (FD_ISSET(ListenSocket, &readfds)) {
			...
			for (int i = 0; i < MAX_CLIENTS; ++i) {
				if (ClientSocket[i] == INVALID_SOCKET) {
					...
					if (err) {
						InfoLog("New client connection init failed: %d", err);
						break;
					}

					InfoLog("New client succesfully added to the list by index %d", i);
					break;
				}
			}
		}
		...
		for (int i = 0; i < MAX_CLIENTS; ++i) {
			...
			if (FD_ISSET(CurrSocket, &readfds)) {
				...
				if (iResult > 0)
					...
					if (iResult == 0)
						InfoLog("Client %d connection closing..", i);
					else
						WSAErrorLog("recv from client %d failed", i);

					int err = CloseClientConnection(i);
					if (err) {
						InfoLog("Client %d connection closing failed: %d", i, err);
						break;
					}
